...
from methods.dos import DOS


def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate images for prompts using SDXL, DOS-SDXL, and DyDOS-SDXL."
    )
    parser.add_argument("--prompts_txt", default="/home/saket_p/multi_object_generation/T2I-CompBench/examples/dataset/subset_eval.txt", help="Path to text file containing prompts.")
    ...
    parser.add_argument("--dos_subdir", default="dos_sdxl", help="Subdirectory name for DOS-SDXL outputs.")
...
def infer_objects_from_prompt(prompt):
    ...
    for chunk in doc.noun_chunks:
        chunk_words = [token.text for token in chunk if token.pos_ != "DET"]
        if chunk_words:
...
def generate_dydos_sdxl(pipe, prompt, objects, seed, args):
    text_embeddings, _, pooled_text_embeddings, _ = pipe.encode_prompt([prompt])
    text_embeddings = text_embeddings.clone().detach()
    pooled_text_embeddings = pooled_text_embeddings.clone().detach()
...
def main():
    ...
    dos_dir = output_root / args.dos_subdir
    ...
    print(f"Saving DOS-SDXL images to: {dos_dir}")
    ...
    for idx, (prompt, objects, line_number) in enumerate(entries):
        if len(objects) < 2:
            print(
                f"[WARN] Line {line_number}: could not infer >=2 exact objects for DOS/DyDOS. "
        ...
        dos_path = dos_dir / filename
        dydos_path = dydos_dir / filename

        if (not args.overwrite) and sdxl_path.exists() and dos_path.exists() and dydos_path.exists():
        ...
        print(f"  DOS/DyDOS objects: {objects}")
        if not dos_path.exists() or args.overwrite:
            image_dos = generate_dos_sdxl(
                pipe,
                prompt=prompt,
                objects=objects,
                seed=current_seed,
            ...
            image_dos.save(dos_path)

        if not dydos_path.exists() or args.overwrite:
            image_dydos = generate_dydos_sdxl(
                pipe,
                prompt=prompt,
                objects=objects,
                seed=current_seed,
