...
from methods.dos import DOS, SDXLTextEncoder
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate images for prompts in a text file using SDXL and DOS-SDXL."
    )
...
def main():
    ...
    for idx, (prompt, objects, line_number) in enumerate(entries):
        if len(objects) < 2:
            print(
                f"[WARN] Line {line_number}: could not infer >=2 exact objects for DOS. "
        ...
        if (not args.overwrite) and sdxl_path.exists() and dos_path.exists():
